...
public class TpaCommand {
                ...
                .then(CommandManager.argument("player", StringArgumentType.word())
                        ...
                        .executes(context -> {
                            ...
                            if (target.getUuid().equals(requester.getUuid())) {
                                CommandUtils.sendBilingual(requester,
                                        Text.literal("自分にテレポートすることができません。").formatted(Formatting.GRAY),
                            ...
                            CommandUtils.sendBilingual(requester,
                                    Text.empty().append(Text.literal(targetName).formatted(Formatting.AQUA)).append(Text.literal("さんにTPAリクエストを送りました。").formatted(Formatting.GRAY)),
                                    Text.empty().append(Text.literal("Sent TPA request to ").formatted(Formatting.GRAY)).append(Text.literal(targetName).formatted(Formatting.AQUA)));
    ...
    public static void sendTpaHelp(ServerPlayerEntity player) {
        ...
        player.sendMessage(Text.literal("/tpa <player>").formatted(Formatting.YELLOW));
        ...
        player.sendMessage(Text.literal("/tpahere <player>").formatted(Formatting.YELLOW));
        ...
        player.sendMessage(Text.literal("/tpa accept [player]").formatted(Formatting.YELLOW));
        ...
        player.sendMessage(Text.literal("/tpa deny [player]").formatted(Formatting.YELLOW));
        ...
        if (requesterName != null) {
            ...
            if (requestToProcess == null) {
                CommandUtils.sendBilingual(player,
                        Text.empty().append(Text.literal(requesterName).formatted(Formatting.AQUA)).append(Text.literal("さんからの保留中のリクエストはありません。").formatted(Formatting.GRAY)),
                        Text.empty().append(Text.literal("You have no pending request from ").formatted(Formatting.GRAY)).append(Text.literal(requesterName).formatted(Formatting.AQUA)).append(Text.literal(".").formatted(Formatting.GRAY))
                );
                return 0;
            }
        } else {
            if (requests.size() > 1) {
                ...
                CommandUtils.sendBilingual(player,
                        Text.empty().append(Text.literal("複数のTPAリクエストがあります: ").formatted(Formatting.GRAY))
                                .append(Text.literal(pendingRequesters).formatted(Formatting.AQUA))
                                .append(Text.literal("。 ").formatted(Formatting.GRAY))
    ...
    private static int acceptRequest(ServerPlayerEntity acceptor, TpaState.TpaRequest request) {
        ...
        if (source == null || destination == null) {
            ...
            CommandUtils.sendBilingual(source,
                    Text.empty().append(Text.literal(destination.getName().getString()).formatted(Formatting.AQUA)).append(Text.literal("さんにテレポートしました！").formatted(Formatting.GRAY)),
                    Text.empty().append(Text.literal("Teleported to ").formatted(Formatting.GRAY)).append(Text.literal(destination.getName().getString()).formatted(Formatting.AQUA)).append(Text.literal("!").formatted(Formatting.GRAY)));
            CommandUtils.sendBilingual(destination,
                    Text.empty().append(Text.literal(source.getName().getString()).formatted(Formatting.AQUA)).append(Text.literal("さんがテレポートしてきました！").formatted(Formatting.GRAY)),
                    Text.empty().append(Text.literal(source.getName().getString()).formatted(Formatting.AQUA)).append(Text.literal(" has teleported to you!").formatted(Formatting.GRAY)));
        }
    ...
    private static int denyRequest(ServerPlayerEntity denier, TpaState.TpaRequest request) {
        ...
        if (originalRequester != null) {
            CommandUtils.sendBilingual(originalRequester,
                    Text.empty().append(Text.literal(denier.getName().getString()).formatted(Formatting.AQUA)).append(Text.literal("さんがリクエストを拒否しました。").formatted(Formatting.GRAY)),
                    Text.empty().append(Text.literal(denier.getName().getString()).formatted(Formatting.AQUA)).append(Text.literal(" denied your request.").formatted(Formatting.GRAY)));
        }
        CommandUtils.sendBilingual(denier,
                Text.empty().append(Text.literal(requesterName).formatted(Formatting.AQUA)).append(Text.literal("さんのリクエストを拒否しました。").formatted(Formatting.GRAY)),
                Text.empty().append(Text.literal("Request from ").formatted(Formatting.GRAY)).append(Text.literal(requesterName).formatted(Formatting.AQUA)).append(Text.literal(" denied.").formatted(Formatting.GRAY)));
